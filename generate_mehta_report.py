"""Generate a comprehensive HTML report for Tyeb Mehta auction analysis.

Uses estimate midpoint as proxy value for unsold lots to supplement
the small number of sold lots (3 of 17).
"""
...
# ── Load & filter data ──────────────────────────────────────────────────────
mehta = pd.read_csv('data/raw/lots_tyeb_mehta_clean.csv')
...
for i, row in all_lots.iterrows():
    ...
    if row['is_sold'] and pd.notna(row['hammer_price_usd']):
        fig.add_trace(go.Scatter(
            ...
            showlegend=(i == all_lots[all_lots['is_sold']].index[0]),
...
stats = {
    ...
    'creation_range': f"{int(mehta['year_created'].dropna().min())}-{int(mehta['year_created'].dropna().max())}",
}
...
<title>Tyeb Mehta - Auction Market Analysis</title>
<style>
  :root {{
...
<footer>
  Tyeb Mehta Auction Market Analysis | Data: Christie's ({stats['years_range']}) | Generated with Python + Plotly
</footer>

</body>
</html>"""

with open('mehta_report.html', 'w') as f:
    f.write(html)

print("Report written to mehta_report.html")
