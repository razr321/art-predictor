#!/usr/bin/env python3
"""Scrape all Tyeb Mehta sold lots from Christie's search results.
...
Output: data/raw/lots_tyeb_mehta.csv
"""
...
logger = setup_logger(__name__, "scrape_tyeb_mehta.log")

OUTPUT_FILE = DATA_RAW / "lots_tyeb_mehta.csv"
PROGRESS_FILE = DATA_RAW / "lots_tyeb_mehta_progress.ndjson"
DELAY = 2.0
...
def main():
    logger.info("=" * 60)
    logger.info("Scraping Tyeb Mehta lots from Christie's search")
    logger.info("=" * 60)
    ...
    try:
        ...
        search_url = (
            ...
            "entry=tyeb%20mehta&page=1&sortby=realized_desc&tab=sold_lots"
        )
        logger.info("Phase 1: Collecting lot URLs from search pages...")
        lot_urls = get_lot_urls_from_search(driver, search_url, max_pages=15)

        # Also get unsold lots
        search_url_unsold = (
            "https://www.christies.com/en/search?"
            "entry=tyeb%20mehta&page=1&sortby=relevance&tab=unsold_lots"
        )
