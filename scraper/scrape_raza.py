#!/usr/bin/env python3
"""Scrape all S.H. Raza lots from Christie's search results.
...
logger = setup_logger(__name__, "scrape_raza.log")

OUTPUT_FILE = DATA_RAW / "lots_raza.csv"
PROGRESS_FILE = DATA_RAW / "lots_raza_progress.ndjson"
DELAY = 2.0
...
def main():
    logger.info("=" * 60)
    logger.info("Scraping S.H. Raza lots from Christie's search")
    logger.info("=" * 60)
    ...
    try:
        ...
        search_url = (
            ...
            "entry=S.H.%20Raza&page=1&sortby=realized_desc&tab=sold_lots"
        )
        ...
        search_url2 = (
            "https://www.christies.com/en/search?"
            "entry=Sayed%20Haider%20Raza&page=1&sortby=realized_desc&tab=sold_lots"
        )
        logger.info("Also searching 'Sayed Haider Raza'...")
        lot_urls.extend(get_lot_urls_from_search(driver, search_url2, max_pages=10))

        # Also get unsold lots
        search_url_unsold = (
            "https://www.christies.com/en/search?"
            "entry=S.H.%20Raza&page=1&sortby=relevance&tab=unsold_lots"
        )
